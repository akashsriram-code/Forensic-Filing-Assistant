...
import { getArg, getIngestionRunStatus, hasArg, isDirectRun, type IngestionSource } from './13f-ingestion-utils';
...
async function main() {
    ...
    if (!quarter || !source || !['live-edgar', 'sec-bulk'].includes(source)) {
        console.error('Usage: npx tsx scripts/check-13f-ingestion-status.ts --quarter 2026-Q1 --source sec-bulk [--github-output]');
        process.exit(1);
    }
    ...
    if (hasArg('--github-output')) {
        ...
        console.log(JSON.stringify({ quarter, source, ...status }, null, 2));
    }
}
